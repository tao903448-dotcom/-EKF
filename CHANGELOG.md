...
## [3.5.1] - 2026-06-21 —— GUI 遥测可信度治理

- 图形仪表盘移除"伪造为真实"的装饰性数值：经纬度、`COV:0.124`、`GNSS Precision
  Fix`、`Gaussian(0,0.5)` 噪声标注、新息均值/方差、`78% 利用率` 改为示意标注；
  分析页 fallback RMSE 由硬编码旧值改为 0（未计算时显示 0 而非假数）。
- GUI 文件头注明：轨迹/估计/新息/RMSE 对比均来自真实滤波，少量装饰指示为示意；
  权威定量评测以命令行 `attitude_demo`/`ekf_demo` 为准。
- 仅字符串/字面量改动，不触及逻辑；C 库与全部 92 测试不受影响。
  （接真实后端量的完整改造需在 Windows 端编译验证，列为后续。）

## [3.5.0] - 2026-06-21 —— MEKF 乘性误差状态对照

### 新增
- **MEKF（乘性误差状态 EKF）** `mekf.{h,c}`：6 维误差状态 `[δθ, δb]`，协方差不对
  单位四元数过参数化；预测传播误差协方差、更新经卡尔曼增益求 δx 并乘性注入
  （q←q⊗δq(δθ)、b←b+δb），是姿态估计的金标准形式，作为加性 7 状态 EKF 的对照。
- 观测雅可比 `H_θ=[h]×`（向量观测），Cholesky 解 6×6 系统，回退通用求逆。
- 测试 `test_mekf.c`：CLEAN RMSE 0.43°、OUTLIER 不发散、**与加性 EKF 点估计等价**
  （|Δ|<0.05°，互证两者实现正确）、**NEES 协方差一致性**（需真值，理想≈6）。

### 结论
- 良态下 MEKF 与加性 EKF 点估计一致；MEKF 优势在 6 自由度协方差的理论严谨性
  （可用 NEES 严格检验），为后续多传感器/长航时扩展打基础。

### 质量
- 测试 88→**92** 全过；`mekf.c` `-Werror` 零告警、ASan/UBSan 干净。

...
- ✅ **质量保障**：92 单元/回归测试全过，ASan/UBSan 零报错，`-Wall -Wextra -Werror` 零警告；GitHub Actions 六作业 CI。
...
│   └── mekf.c          乘性误差状态 EKF
├── tests/              matrix(17) / ekf(51) / attitude(20) / mekf(4) + benchmark
├── examples/
...
| `test_attitude` | 20 / 20 | 四元数运算、姿态精度与 NIS 一致性、鲁棒性、加速度门控 |
...
- [x] 乘性误差状态 EKF（MEKF）对照实现（6 维误差状态，NEES 一致性）
- [ ] 过程噪声 Q 在线自适应、运动模型融合

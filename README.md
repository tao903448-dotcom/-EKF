...
- 🛡️ **鲁棒 + 自适应见真章**：振动野值下姿态 RMSE **↓79%**、机动失配下 **↓70%**（自适应+加速度门控），并把一致性指标 NIS 从 73 拉回 ≈4。参数由算力服务器 64 核蒙特卡洛寻优。
...
- ✅ **质量保障**：69 单元/回归测试全过，ASan/UBSan 零报错，`-Wall -Wextra -Werror` 零警告；GitHub Actions 六作业 CI（gcc / clang / 严格告警 / ASan-UBSan / ARM 交叉编译 / cppcheck 静态分析）。
...
> **加速度自适应门控**（按比力幅值偏离 g 降权加速度，参数经 64 核 MC 寻优）进一步压低失配残差：
> MANEUVER 自适应 13.8°→**7.67°**（叠加后相对标准 **↓70%**），OUTLIER 标准 1.75°→0.68°，
> CLEAN 不劣化（门控/自适应"按需触发"）。详见报告 §5.1–5.2。
>
...
├── tests/              matrix(14) / ekf(42) / attitude(13) + benchmark
├── examples/
...
| `test_ekf` | 42 / 42 | 协方差不坍缩、Standard≡Joseph、抗野值、预测不混叠、枚举守卫、双精度参考对照 |
